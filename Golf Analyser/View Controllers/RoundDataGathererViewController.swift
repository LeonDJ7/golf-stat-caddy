//
//  RoundDataGathererViewController.swift
//  Golf Analyser
//
...
class RoundDataGathererViewController: UIViewController {

 
    
    
    @IBOutlet weak var backBtnToHoleTypeVC: UIButton!
    @IBOutlet weak var backBtnToLastQuestion: UIButton!
    ...
    @IBAction func confirmBtnTapped(_ sender: Any) {
     Firestore.firestore().collection("Users").document(Auth.auth().currentUser!.uid).collection("Rounds").getDocuments { (querySnapshot, error) in
            ...
            Firestore.firestore().collection("Users").document(Auth.auth().currentUser!.uid).collection("Rounds").document("\(roundNumber)").collection("Holes").getDocuments { (querySnapshot, error) in
                ...
                holeNumber = querySnapshot!.documents.count + 1
                ...
                // if its their first round, make it 1 instead of the default 0 and set the first round to ["doesCount" : false]
                if roundNumber == 0 {
                    roundNumber = 1
                    Firestore.firestore().collection("Users").document(Auth.auth().currentUser!.uid).collection("Rounds").document("\(roundNumber)").setData(["doesCount" : false])
                }
                ...
                if holeNumber == 9 {
                    // check whether user is playing 9 or 18
                    self.performSegue(withIdentifier: "toContinueOrNotSegue", sender: self)
                }
                
                if holeNumber == 18 {
                    self.performSegue(withIdentifier: "toConfirmAfterEighteen", sender: self)
                }
                
                if holeNumber != 9 && holeNumber != 18 {
                    self.performSegue(withIdentifier: "unwindSegueToHoleType", sender: self)
                }
            }
        }
    }
